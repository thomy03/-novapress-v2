...
# ==========================================
# Nexus Timeline (Scroll-Driven Images)
# ==========================================

class NexusTimelineImageResponse(BaseModel):
    url: str
    timestamp: float
    synthesis_id: str
    synthesis_title: str
...
class NexusTimelineResponse(BaseModel):
    topic: str
    images: List[NexusTimelineImageResponse]
    total: int
...
async def get_nexus_timeline(topic_name: str):
    """
    Get chronological list of nexus images for the scroll-driven viewer.

    Returns images ordered by timestamp (oldest first) for a given topic.
    These images are generated by the pipeline when causal graphs have >= 3 nodes.
    """
    try:
        ...
        images_raw = await generator.get_timeline_images(topic_slug)

        images = [
            NexusTimelineImageResponse(
                url=img.get("url", ""),
                timestamp=img.get("timestamp", 0),
                synthesis_id=img.get("synthesis_id", ""),
                synthesis_title=img.get("synthesis_title", ""),
                node_count=img.get("node_count", 0),
                topic=img.get("topic", topic_name),
            )
