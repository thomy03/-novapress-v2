...
# ==========================================
# Phase 7: Predictions Schemas
# ==========================================

class PredictionResponse(BaseModel):
    """A single prediction/future scenario"""
    prediction: str
    probability: float  # 0-1
    type: str  # economic, political, social, geopolitical, tech
    timeframe: str  # court_terme, moyen_terme, long_terme
    rationale: str


class PredictionsResponse(BaseModel):
    """All predictions for a synthesis"""
    synthesis_id: str
    title: str
    predictions: List[PredictionResponse]
    count: int
    types: List[str]  # Distinct prediction types found
...
async def get_causal_graph(synthesis_id: str):
    ...
    try:
        ...
        # Enrich with keyword nodes (Phase 7)
        try:
            bridge = get_keyword_causal_bridge()
            key_entities = synthesis.get("key_entities", [])
            synthesis_text = synthesis.get("summary", "") + " " + synthesis.get("body", "")
            causal_data = bridge.enrich_causal_graph(
                causal_data,
                key_entities,
                synthesis_text
            )
        except Exception as e:
            logger.warning(f"Failed to enrich causal graph with keywords: {e}")
...
# ==========================================
# Phase 7: Predictions Endpoint
# ==========================================

@router.get("/syntheses/{synthesis_id}/predictions", response_model=PredictionsResponse)
async def get_predictions(synthesis_id: str):
    """
    Get predictions/future scenarios for a synthesis.

    Returns predictions that were generated by the LLM during synthesis creation.
    Each prediction includes:
    - prediction: The scenario description
    - probability: Likelihood (0-1)
    - type: Category (economic, political, social, geopolitical, tech)
    - timeframe: When it might occur (court_terme, moyen_terme, long_terme)
    - rationale: Reasoning behind the prediction

    These predictions are pre-computed (no LLM call at display time).
    """
    try:
        qdrant = get_qdrant_service()
        ...
        # Get predictions from causal_graph JSON
        causal_data = synthesis.get("causal_graph", {})

        # Handle both string and dict formats
        if isinstance(causal_data, str):
            import json
            try:
                causal_data = json.loads(causal_data)
            except json.JSONDecodeError:
                causal_data = {}

        predictions_raw = causal_data.get("predictions", [])
        ...
        for p in predictions_raw:
            if isinstance(p, dict):
                ...
                predictions.append(PredictionResponse(
                    ...
                    type=pred_type,
                    timeframe=p.get("timeframe", "moyen_terme"),
        ...
        logger.error(f"Failed to get predictions for synthesis {synthesis_id}: {e}")
        # Return empty predictions instead of error (graceful fallback)
        return PredictionsResponse(
            synthesis_id=synthesis_id,
            title="",
            predictions=[],
...
class GeoSvgResponse(BaseModel):
    synthesis_id: str
    svg_content: Optional[str] = None
    has_geo: bool


@router.get("/syntheses/{synthesis_id}/geo-svg", response_model=GeoSvgResponse)
async def get_geo_svg(synthesis_id: str):
    """
    Get pre-generated editorial geo SVG map for a synthesis.

    These maps are generated during pipeline execution when geographic
    context is present. No LLM calls at display time.
    """
    try:
        generator = get_nexus_image_generator()
        svg_content = await generator.get_geo_svg(synthesis_id)

        return GeoSvgResponse(
            synthesis_id=synthesis_id,
            svg_content=svg_content,
            has_geo=svg_content is not None,
        )

    except Exception as e:
        logger.error(f"Failed to get geo SVG for {synthesis_id}: {e}")
        return GeoSvgResponse(
            synthesis_id=synthesis_id,
            svg_content=None,
            has_geo=False,
...
# ==========================================
# Nexus Timeline (Scroll-Driven SVG Infographics)
# ==========================================

class NexusTimelineEntry(BaseModel):
    svg_content: str
    timestamp: float
    synthesis_id: str
    synthesis_title: str
...
class NexusTimelineResponse(BaseModel):
    topic: str
    entries: List[NexusTimelineEntry]
    total: int


@router.get("/topics/{topic_name}/nexus-timeline", response_model=NexusTimelineResponse)
async def get_nexus_timeline(topic_name: str):
    """
    Get chronological list of nexus SVG infographics for the scroll-driven viewer.

    Returns SVGs ordered by timestamp (oldest first) for a given topic.
    These are generated by the pipeline when causal graphs have >= 3 nodes.
    """
    try:
        generator = get_nexus_image_generator()

        # Build the same slug the generator uses
        topic_slug = generator._build_topic_slug(topic_name)

        svgs_raw = await generator.get_timeline_svgs(topic_slug)

        entries = [
            NexusTimelineEntry(
                svg_content=svg.get("svg_content", ""),
                timestamp=svg.get("timestamp", 0),
                synthesis_id=svg.get("synthesis_id", ""),
                synthesis_title=svg.get("synthesis_title", ""),
                node_count=svg.get("node_count", 0),
                topic=svg.get("topic", topic_name),
                has_geo=svg.get("has_geo", False),
                has_metrics=svg.get("has_metrics", False),
            )
            for svg in svgs_raw
            if svg.get("svg_content")
        ]

        return NexusTimelineResponse(
            topic=topic_name,
            entries=entries,
            total=len(entries),
        )
        ...
        logger.error(f"Failed to get nexus timeline for '{topic_name}': {e}")
        return NexusTimelineResponse(
            topic=topic_name,
            entries=[],
            total=0,
